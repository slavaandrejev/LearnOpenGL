...
bool OpenGLRender::on_render(const Glib::RefPtr<Gdk::GLContext>& context) {
    ...
    auto radius = 10.0f;
    auto cameraX = radius * std::sin(curTime);
    auto cameraY = radius * std::cos(curTime);

    auto view       = glm::lookAt(glm::vec3(cameraX, 0.0, cameraY), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0));
