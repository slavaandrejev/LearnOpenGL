...
bool OpenGLRender::on_render(const Glib::RefPtr<Gdk::GLContext>& context) {
    ...
    lightingShader->set("lightPos", glm::vec3(view * glm::vec4(lightPos, 1.0f)));
    auto cubeModel = glm::mat4(1.0f);
    auto normalMatrix = glm::transpose(glm::inverse(glm::mat3(view * cubeModel)));
...
void OpenGLRender::on_realize() {
    ...
    lightingShader = std::make_unique<Shader>(
        ...
        "/ex-003-fs.glsl", GL_FRAGMENT_SHADER);
