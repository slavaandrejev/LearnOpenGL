...
bool OpenGLRender::on_render(const Glib::RefPtr<Gdk::GLContext>& context) {
    ...
    auto 𝛼 = 𝜔 / 60.0f * two_pi<float>() * curTime;
    auto lightRotate = glm::rotate(glm::mat4(1.0f), 𝛼, glm::vec3{0.0f, 1.0f, 0.0f});

    lightingShader->use();
    lightingShader->set("view", view);
    lightingShader->set("projection", projection);
    lightingShader->set("viewPos", camera.GetPosition());
    lightingShader->set("lightPos", glm::vec3{lightRotate * glm::vec4{lightPos, 1.0f}});

    glBindVertexArray(cubeVAO);
    glDrawArrays(GL_TRIANGLES, 0, 36);

    auto lightModel = glm::translate(lightRotate, lightPos);
...
void OpenGLRender::on_realize() {
    ...
    lightingShader = std::make_unique<Shader>(
        "/ex-001-vs.glsl", GL_VERTEX_SHADER,
        "/ex-001-fs.glsl", GL_FRAGMENT_SHADER);
